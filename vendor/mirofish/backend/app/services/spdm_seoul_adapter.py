...
def build_simulation_parameters(
    ...
    for agent_id, persona in enumerate(SEOUL_PERSONAS):
        agent_configs.append(
            AgentActivityConfig(
                ...
                cluster_id=persona["entity_type"],
                cluster_role="leader",
                reasoning_weight=0.9 if persona["entity_type"] in {"GovernmentOfficial", "OnlineCommunityUser"} else 0.6,
                rule_profile="institutional" if persona["entity_type"] == "GovernmentOfficial" else "grassroots",
                sensitivity_metrics=["crowding", "traffic", "consumption", "acceptance"],
                trust_channels=["official-briefing", "social-feed"] if persona["entity_type"] == "GovernmentOfficial" else ["community", "social-feed"],
            )
        )
    ...
    return SimulationParameters(
        ...
        reasoning_config=ReasoningDecimationConfig(
            cluster_count=len(agent_configs),
            cluster_calls_per_round=min(3, len(agent_configs)),
            agent_calls_per_round=min(6, len(agent_configs)),
            cooldown_rounds=int(os.getenv("SPDM_REASONING_COOLDOWN_ROUNDS", "2")),
            rule_agent_share=float(os.getenv("SPDM_AGENT_RULE_SHARE", "0.9")),
            context_memory_limit=int(os.getenv("SPDM_CONTEXT_MEMORY_LIMIT", "2")),
            issue_limit=int(os.getenv("SPDM_CONTEXT_ISSUE_LIMIT", "3")),
            max_llm_concurrency=min(8, len(agent_configs)),
        ),
